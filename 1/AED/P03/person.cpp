...
class person
{
  private:
    ...
    char d_number[max_size + 1];
  public:
    person(const char *name = "",const char *address = "", const char *number = ""); // constructor
...
void person::set_number(const char *number)
{
  int i;

  for(i = 0;number[i] != '\0' && i < max_size;i++)
    d_number[i] = number[i];
  d_number[i] = '\0';
}

person::person(const char *name,const char *address,const char *number)
{ // constructor
  if(debug != 0)
...
inline const char *person::get_number(void)
{
  return d_number;
}
...
int main(void)
{
  ...
  person q("Daniel Pedrinho","no address","no number");         // create a new object (no copy needed)

  cout << "Before change of address: " << p.get_name() << ", " << p.get_address() << ", " << p.get_number() << endl;
  p.set_address("bench 24, Central Park, New York");
  p.set_number( "961119150");
  cout << "After change of address:  " << p.get_name() << ", " << p.get_address() << ", " << p.get_number() << endl;
  
  cout << "Before change of address: " << q.get_name() << ", " << q.get_address() << ", " << q.get_number() << endl;
  q.set_address("Rua das Camélias, Edifício Lomba, Casa E");
  q.set_number( "918209449");
  cout << "After change of address:  " << q.get_name() << ", " << q.get_address() << ", " << q.get_number() << endl;
