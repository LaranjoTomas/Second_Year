...
void do_it(int N)
{
  cout << " n n*n      sqrt(n)       n^3       cbrt(n*n)\n"
       << "-- --- ----------------- ---- ---------------------\n";
  for(int i = 1;i <= N;i++)
  {
    cout << setw(2) << i
         ...
         << setw(4) << i * i * i
         << " "
         << scientific << setw(17) << setprecision(15) << cbrt(double(i*i))
         << endl;
#if ALSO_USE_PRINTF != 0
    // same as above, but now using the printf function
    printf("%2d %3d %17.15f %4d %17.15f\n",i,i * i,sqrt(double(i)),i * i * i, cbrt(double(i*i)));
#endif
  }
}
