		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				if (line.startsWith("#")) {
					...
					parts = line.split(",");
					Client client = new Client(clientName, clientLocation);
					event = em.addEvent(client, date);
				} else if (line.startsWith("*")) {
					String[] parts = line.split(",");
					String Activity = parts[0];
					String Option = parts[1];
					int Participants = Integer.parseInt(parts[2]);
					if (Activity.equals("Sport")) {
						event.addActivity(new Sport(Sport.Modality.valueOf(Option), Participants));
					} else if (Activity.equals("Catering")) {
						event.addActivity(new Catering(Catering.Option.valueOf(Option), Participants));
					} else if (Activity.equals("Culture")) {
						event.addActivity(new Culture(Culture.Option.valueOf(Option), Participants));
					}
				}
			}
			sc.close();
