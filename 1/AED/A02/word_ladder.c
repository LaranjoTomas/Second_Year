...
static hash_table_t *hash_table_create(void)
{
  ...
  hash_table->hash_table_size = 100u; // a prime number
...
static void hash_table_grow(hash_table_t *hash_table)
{
  ...
  new_heads = (hash_table_node_t **)malloc(2u * hash_table->hash_table_size * sizeof(hash_table_node_t *));
  if(new_heads == NULL)
  {
    fprintf(stderr,"hash_table_grow: out of memory\n");
    exit(1);
  }
  for(i = 0u;i < 2u * hash_table->hash_table_size;i++) {
    new_heads[i] = NULL;
  }
  for(i = 0u;i < hash_table->hash_table_size;i++) {
    for(node = hash_table->heads[i];node != NULL;node = next)
    {
      next = node->next;
      node->next = new_heads[crc32(node->word) % (2u * hash_table->hash_table_size)];
      new_heads[crc32(node->word) % (2u * hash_table->hash_table_size)] = node;
    }
...
static hash_table_node_t *find_word(hash_table_t *hash_table,const char *word,int insert_if_not_found)
{
  ...
  if(insert_if_not_found)
  {
    ...
    strncpy(word,node->word,_max_word_size_);
    node->visited = 0;
    node->previous = NULL;
...
{
  // find a path from from_word to to_word
